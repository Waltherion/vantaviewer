...
void main()
{
    ...
    // Image is stored in its native primaries; convert to the BT.709-primaries
    // surface for display. Clamp out-of-709-gamut negatives for safe display (export
    // keeps the full native-gamut data via a separate path).
    if (u.primaries > 1.5)
